...
namespace Symbol.Data {
    ...
    /// <summary>
    /// Npgsql数据库提供者(PostgreSQL)
    /// </summary>
    public class NpgsqlDatabaseProvider : DatabaseProvider {
        ...
        static NpgsqlDatabaseProvider() {
            ...
            _types.Add("Npgsql.NpgsqlConnection", null);
            _types.Add("Npgsql.NpgsqlConnectionStringBuilder", null);
            _types.Add("NpgsqlTypes.NpgsqlDbType", null);
        }
        /// <summary>
        ...
        public override IDbConnection CreateConnection(string connectionString) {
            ...
            return (IDbConnection)FastWrapper.CreateInstance(type, connectionString);// new Npgsql.NpgsqlConnection(connectionString);
        }
        /// <summary>
        /// 创建数据库连接。
        /// </summary>
        ...
        public override IDataContext CreateDataContext(IDbConnection connection) {
            return new NpgsqlDataContext(this, connection);
        }
        #region PreName
        /// <summary>
        ...
        #region types
        class NpgsqlDataContext : Symbol.Data.DataContext, IDataContext {

            #region ctor
            /// <summary>
            /// 创建 NpgsqlDataContext 的实例
            /// </summary>
            /// <param name="provider">数据库提供者</param>
            /// <param name="connection">数据库连接</param>
            public NpgsqlDataContext(IDatabaseProvider provider, IDbConnection connection) : base(provider, connection) {
            }
            #endregion
                    ...
                    if (TypeExtensions.IsAnonymousType(type) || TypeExtensions.IsInheritFrom(type,typeof(System.Collections.Generic.IDictionary<string, object>))) {
                        CommandParameter p = new CommandParameter() {
                            ...
                            Properties = Symbol.Collections.Generic.NameValueCollection<object>.As(new {
                                NpgsqlDbType = TypeExtensions.Convert("Json", npgsqlDbType),
                            })
                        };
                        param2 = p;
                    } else if (type.IsEnum) {
                        ...
                        CommandParameter p = new CommandParameter() {
                            ...
                            Value = value,
                            Properties = Symbol.Collections.Generic.NameValueCollection<object>.As(new {
                                NpgsqlDbType = TypeExtensions.Convert("Array,Text", npgsqlDbType),
                            })
                        };
                        param2 = p;
                    }
            ...
            public override object ExecuteScalar(string commandText, System.Action<IDbCommand> action, params object[] @params) {
                lock (this) {
                    ...
                    if (commandText.IndexOf("insert ", System.StringComparison.OrdinalIgnoreCase) != -1) {
                        ...
                        if (commandText.IndexOf("returning", System.StringComparison.OrdinalIgnoreCase) == -1) {
                            commandText2 += "returning id";
                        }
                    }
            ...
            public override bool TableSpaceCreate(string name, string path = null) {
                ...
                ExecuteBlockQuery(string.Format("CREATE TABLESPACE \"{0}_{1}\" LOCATION '{2}';", _databaseName, name, path));
            ...
            public override void TableSpaceDelete(string name) {
                ...
                if (!TableSpaceExists(name))
                    return;
                ExecuteBlockQuery(string.Format("drop TABLESPACE \"{0}_{1}\";", _databaseName, name));
            }
            #endregion
...
ORDER BY
    col.table_name,col.ordinal_position;
        ...
        class SelectCommandBuilder : Symbol.Data.SelectCommandBuilder, ISelectCommandBuilder {
            ...
            /// <returns></returns>
            protected override string LikeValueFilter(string value, bool left, bool right, bool reverse) {
                if (reverse)
                    return value;
            ...
            /// <param name="reverse">倒转，为true时表示value like field。</param>
            /// <returns></returns>
            protected override string LikeGrammar(string field, bool left, bool right, bool reverse) {
                if (reverse) {
                    field = PreName(field);
                    if (!left)
                        field = "'^'+" + field;
                ...
                bool isJson = TypeExtensions.Convert<bool>(ConstAttributeExtensions.Const(propertyInfo, "SaveAsJson"), false);
                {
                    ...
                    if (!string.IsNullOrEmpty(dataTypeName)) {
                        if (string.Equals(dataTypeName, "jsonb", System.StringComparison.OrdinalIgnoreCase)) {
                            isJson = true;
                            dbType = "Jsonb";// NpgsqlTypes.NpgsqlDbType.Jsonb;
                        } else if (string.Equals(dataTypeName, "json", System.StringComparison.OrdinalIgnoreCase)) {
                            isJson = true;
                            dbType = "Json";// NpgsqlTypes.NpgsqlDbType.Json;
                        }
                    }
                    if (npgsqlDbType != null && dbType != null) {
                        commandParameter.Properties = Symbol.Collections.Generic.NameValueCollection<object>.As(new {
                            NpgsqlDbType = TypeExtensions.Convert(dbType, npgsqlDbType),
                        });
                    }
                }
                if (!isJson) {
            ...
            protected override void PreValues() {
                ...
                foreach (string key in _fields.AllKeys) {
                    ...
                    CommandParameter p = new CommandParameter() {
                        ...
                        Value = value == null ? null : Symbol.Serialization.Json.ToString(value, true),
                        Properties = Symbol.Collections.Generic.NameValueCollection<object>.As(new {
                            NpgsqlDbType = TypeExtensions.Convert("Json", GetDbType()),
                        })
                    };
                    _fields[key] = p;
                }
