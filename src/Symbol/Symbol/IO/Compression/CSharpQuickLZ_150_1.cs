...
namespace Symbol.IO.Compression {
    ...
    /// QuickLZ
    /// </summary>
    public class CSharpQuickLZ_150_1 {

        #region consts

        private const int QLZ_VERSION_MAJOR = 1;
        private const int QLZ_VERSION_MINOR = 5;
        private const int QLZ_VERSION_REVISION = 0;

        // Streaming mode not supported
        private const int QLZ_STREAMING_BUFFER = 0;

        // Bounds checking not supported  Use try...catch instead
        private const int QLZ_MEMORY_SAFE = 0;
        ...
        #region header size write
        private static int headerLen(byte[] source) {
            return ((source[0] & 2) == 2) ? 9 : 3;
        }

        static int sizeDecompressed(byte[] source) {
        ...
        static int sizeCompressed(byte[] source) {
            if (headerLen(source) == 9)
        ...
        /// </summary>
        /// <param name="source"></param>
        /// <param name="level">µ»º∂£∫1°¢3</param>
        /// <returns></returns>
