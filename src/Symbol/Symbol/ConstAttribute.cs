...
public static class ConstAttributeExtensions {
    ...
    private static readonly System.Collections.Concurrent.ConcurrentDictionary<ICustomAttributeProvider, IDictionary<string, string>> _list = new System.Collections.Concurrent.ConcurrentDictionary<ICustomAttributeProvider, IDictionary<string, string>>();
    ...
    static IDictionary<string, string> GetValues(ICustomAttributeProvider provider) {
        Func<ICustomAttributeProvider, IDictionary<string, string>> func = (p) => {
    ...
    static IDictionary<string, string> GetValues(MethodInfo methodInfo, ParameterInfo parameter) {
        return _list.GetOrAdd(parameter, (p) => {
            var list = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
            GetValues(methodInfo, parameter, list);
            return list;
        });
    }
    static void GetValues(MethodInfo methodInfo, ParameterInfo parameter, IDictionary<string, string> list) {
    ...
    static void GetValues(ICustomAttributeProvider provider, IDictionary<string, string> list, bool inherit) {
        if (inherit) {
            {
        ...
        }
        {
            ...
            if (attributes != null) {
                foreach (ConstAttribute p in attributes) {
                    IDictionaryExtensions.SetValue(list, p.Key, p.Value);
                }
            }
        }
        bool isEmptyText = string.IsNullOrEmpty(IDictionaryExtensions.GetValue(list, "Text"));
        bool isEmptyDisplayName = true;
        {
            ...
            if (attributes != null) {
                foreach (System.ComponentModel.DisplayNameAttribute p in attributes) {
                    if (!string.IsNullOrEmpty(p.DisplayName)) {
                        ...
                        IDictionaryExtensions.SetValue(list, "DisplayName", p.DisplayName);
                        if (isEmptyText) {
                            isEmptyText = false;
                            IDictionaryExtensions.SetValue(list, "Text", p.DisplayName);
                        }
                    }
                }
            }
            if (isEmptyDisplayName) {
                ...
                IDictionaryExtensions.SetValue(list, "DisplayName", IDictionaryExtensions.GetValue(list, "Text"));
            }
        }
        {
            ...
            if (attributes != null) {
                foreach (System.ComponentModel.DescriptionAttribute p in attributes) {
                    if (!string.IsNullOrEmpty(p.Description)) {
                        isEmptyDisplayName = false;
                        IDictionaryExtensions.SetValue(list, "Description", p.Description);
                        if (isEmptyText) {
                            isEmptyText = false;
                            IDictionaryExtensions.SetValue(list, "Text", p.Description);
                        }
                        if (isEmptyDisplayName) {
                            isEmptyDisplayName = false;
                            IDictionaryExtensions.SetValue(list, "DisplayName", p.Description);
                        }
                    }
                }
            }
    ...
    public static string ConstParameter(
        ...
        if (parameter == null)
            return "";
        return IDictionaryExtensions.GetValue(GetValues(methodInfo, parameter), string.IsNullOrEmpty(key) ? "Text" : key) ?? "";
    }
    #endregion
    ...
    public static string Const(
        ...
        return IDictionaryExtensions.GetValue(list, string.IsNullOrEmpty(key) ? "Text" : key) ?? "";
    }
    #endregion
    ...
    public static string Const(
        ...
        if (provider == null)
            return "";
        return IDictionaryExtensions.GetValue(GetValues(provider), string.IsNullOrEmpty(key) ? "Text" : key) ?? "";
    }
    #endregion
