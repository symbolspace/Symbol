...
public class FastObject {
    ...
    /// 默认BindingFlags
    /// </summary>
    public const BindingFlags DefaultBindingFlags = BindingFlags.Public | BindingFlags.NonPublic;
    ...
    public BindingFlags GetBindingFlags(BindingFlags flags, bool ignoreCase) {
    ...
    public T Get<T>(string path) {
        return ConvertExtensions.Convert<T>(Path(_instance, path));
    }
    /// <summary>
    /// 获取值
    /// </summary>
    ...
    public T Get<T>(string path, params object[] formatArgs) {
        return ConvertExtensions.Convert<T>(Path(_instance, FormatPath(path, formatArgs)));
    }
    /// <summary>
    /// 获取值
    /// </summary>
    ...
    public T Get<T>(string path, T defaultValue) where T : struct {
        return ConvertExtensions.Convert<T>(Path(_instance, path), defaultValue);
    }
    /// <summary>
    /// 获取值
    /// </summary>
    ...
    public T Get<T>(string path, T defaultValue, params object[] formatArgs) where T : struct {
        return ConvertExtensions.Convert<T>(Path(_instance, FormatPath(path, formatArgs)), defaultValue);
    }
    /// <summary>
    /// 设置值
    /// </summary>
                        ...
                        if (list != null) {
                            ...
                            try {
                                if (string.Equals("add", p, System.StringComparison.OrdinalIgnoreCase)
                                    ...
                                    lastTarget = list[ConvertExtensions.Convert<int>(p, 0)];
                                }
                            } catch { lastTarget = null; }
                            lastKey = p;
                            lastAllowNull = true;
                            setMethod = (p1, p2, p3, p4) => {
                                //((System.Collections.Generic.IList<object>)p1)[ConvertExtensions.Convert<int>(p3, 0)] = p4;
                                ...
                                int p11_list = ConvertExtensions.Convert<int>(p3, 0);
                ...
                if (p11.StartsWith("\"")) {
...

                    {
                        ...
                        if (list != null) {
                            parent = lastTarget;
                            try {
                                lastTarget = list[ConvertExtensions.Convert<int>(p11, 0)];
                            } catch { lastTarget = null; }
                            lastKey = p11;
                            lastAllowNull = true;
                            setMethod = (p1, p2, p3, p4) => {
                    ...
                    }
                    {
                        ...
                        if (list != null) {
                            ...
                            setMethod = (p1, p2, p3, p4) => {
                                ...
                                p10_list[p11_list] = p4;
                                //((System.Collections.IList)p1)[ConvertExtensions.Convert<int>(p3, 0)] = p4;
                            };
                            continue;
                        }
                    }
                    {
                        ...
                        if (list != null) {
                            parent = lastTarget;
                            lastTarget = list.GetValue(ConvertExtensions.Convert<int>(p11, 0));
                            lastKey = p11;
                            lastAllowNull = true;
                            setMethod = (p1, p2, p3, p4) => ((System.Array)p1).SetValue(p4, ConvertExtensions.Convert<int>(p3, 0));
                            continue;
                        }
                    }
                }
    ...
    public static IDictionary<string, object> As(object values) {
        if (values is IDictionary<string, object> result)
            return result;
        result = new Dictionary<string, object>(StringComparer.OrdinalIgnoreCase);
        IDictionaryExtensions.SetValues(result, values);
        return result;
    }
    /// <summary>
    /// 将多个对象组合在一起（仅限第一层属性）
    ...
    /// <returns>不会有null。</returns>
    public static IDictionary<string, object> Combine(params object[] objects) {
    ...
    /// <param name="objects">用于组装的对象列表，实体类/匿名类/字典/JSON</param>
    /// <returns>不会有null。</returns>
    public static IDictionary<string, object> Combine(System.Collections.IEnumerable objects) {
        var values = As(null);
        if (objects != null) {
            foreach (var item in objects) {
                IDictionaryExtensions.SetValues(values, item);
            }
        }
        return values;
    }
    ...
    /// <remarks>type为空时直接返回为null。</remarks>
    ...
    class PathMapper {
        ...
        bool Map_Index(PathNode node) {
            ...
            if (System.Text.RegularExpressions.Regex.IsMatch(node.key, "^[0-9]+$")) {
                int index = ConvertExtensions.Convert<int>(node.key);
                if (
