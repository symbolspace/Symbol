...
/// 文本提取辅助类。
/// </summary>
public static class StringExtractHelper {
    ...
    public static string TagReplace(
#if !NET20
    this 
#endif
    string html, string tagName, string replace) {
        if (string.IsNullOrEmpty(html))
            return string.Empty;
        return System.Text.RegularExpressions.Regex.Replace(html, $"(<[/]*)({tagName})([^>]*>)", $"$1{replace}$3", System.Text.RegularExpressions.RegexOptions.IgnoreCase);
    }
    ...
    this 
#endif
    string html, bool clearNbsp, bool trim) {
        if (string.IsNullOrEmpty(html))
            return string.Empty;
        string result = System.Text.RegularExpressions.Regex.Replace(html, "<[^>]*>", string.Empty, System.Text.RegularExpressions.RegexOptions.IgnoreCase | System.Text.RegularExpressions.RegexOptions.Singleline);
    ...
    public static System.Collections.Generic.IEnumerable<string> RulesStringsStartEnd(
#if !NET20
    this 
#endif
    string text, string start, params string[] ends) {
    ...
    this 
#endif
    string text, string start, int startIndex, params string[] ends) {
    ...
    this 
#endif
    string text, string start, string[] ends, int startIndex, bool isCantionStart, bool isCantionEnd, bool isCase) {
        int endIndex = 0;
    ...
    public static string StringsStartEnd(
#if !NET20
    this 
#endif
    string text, string start, params string[] ends) {
    ...
    this 
#endif
    string text, string start, string[] ends, int startIndex, bool isCantionStart, bool isCantionEnd, bool isCase, out int endIndex) {
    ...
    this 
#endif
    string text, ref string find, bool isCase) {
    ...
    this 
#endif
    string text, ref string find, int startIndex, bool isCase) {
        ...
        if (find.IndexOf("[*]") > 0) {
            ...
            string regex = find.Replace("[*]", ".*");
            var regexOptions = isCase ? System.Text.RegularExpressions.RegexOptions.None : System.Text.RegularExpressions.RegexOptions.IgnoreCase;

            if (!System.Text.RegularExpressions.Regex.IsMatch(content, regex, regexOptions))
                return -1;
            var match = System.Text.RegularExpressions.Regex.Match(content, regex, regexOptions);
