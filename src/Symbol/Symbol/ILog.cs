...
public class LogBase : ILog {
    ...
    private HashSet<string> _levels;
    ...
    public LogBase(string name) {
        Name = name;
        _levels = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
    }
    #endregion
...
public static class FileLogExtenstions {
...
#endif
        System.Type type) {
        return global::Symbol.FileLog.Create(type);
    }
    /// <summary>
    /// 创建日志存储对象（类型）。
    /// </summary>
    ...
    public static FileLog FileLog(this System.Type type, string path) {
        return global::Symbol.FileLog.Create(type, path);
    }
    /// <summary>
    /// 创建日志存储对象（成员，当前程序目录）。
    /// </summary>
    ...
    public static FileLog FileLog(this System.Reflection.MemberInfo memberInfo) {
        return global::Symbol.FileLog.Create(memberInfo);
    }
    /// <summary>
    /// 创建日志存储对象（成员）。
    /// </summary>
    ...
    public static FileLog FileLog(this System.Reflection.MemberInfo memberInfo, string path) {
        return global::Symbol.FileLog.Create(memberInfo, path);
    }
