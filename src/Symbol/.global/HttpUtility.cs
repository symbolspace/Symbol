...
public sealed class HttpUtility {
    ...
    public static byte[] UrlEncodeToBytes(byte[] data, int offset, int count) {
        ...
        if ((offset < 0) || (offset > data.Length - 1)) {
            Symbol.CommonException.ThrowArgumentOutOfRange("offset", offset, 0, data.Length - 1);
        }
        if ((count < 0) || ((offset + count) > data.Length)) {
            Symbol.CommonException.ThrowArgumentOutOfRange("count", offset, 0, data.Length - offset);
        }
